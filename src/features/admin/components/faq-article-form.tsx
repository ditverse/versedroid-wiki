...
export function FaqArticleForm({ mode, articleId, categories, initial }: FaqFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            {error && (
                <div className="mb-6 rounded-lg border border-vd-danger/30 bg-vd-danger-surface p-3 text-sm text-vd-danger">
                    {error}
                </div>
            )}
                                    ...
                                    {translations[locale].faqItems.map((item, i) => (
                                        <div key={i} className="rounded-lg border border-vd-border p-3 space-y-2">
                                            ...
                                            <Button
                                                ...
                                                className="text-vd-danger hover:text-vd-danger/80"
                                            >
                                                Remove
                                            </Button>
                                        </div>
                ...
                <div className="w-full md:w-72 shrink-0 space-y-4">
                    ...
                    <div className="rounded-lg border border-vd-border bg-vd-bg-secondary p-4 space-y-4">
                        ...
                        <div className="flex flex-col gap-2 pt-1">
                            ...
                            <Button type="button" variant="outline" onClick={() => router.push("/admin/faq")} className="w-full border-vd-border">Cancel</Button>
                        </div>
                    </div>
                    ...
                    <div className="rounded-lg border border-vd-border bg-vd-bg-secondary p-4 space-y-3">
                        ...
                        <div className="space-y-2">
                            <Label>Slug</Label>
                            <Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="my-article-slug" className="border-vd-border bg-vd-bg-primary" required />
                        </div>
                        <div className="space-y-2">
                            <Label>Icon (emoji)</Label>
                            <Input value={icon} onChange={(e) => setIcon(e.target.value)} placeholder="📄" className="border-vd-border bg-vd-bg-primary" />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Select value={categoryId} onValueChange={setCategoryId}>
                                ...
                                <SelectContent>
                                    {categories.map((cat) => (
                                        <SelectItem key={cat.id} value={cat.id}>{cat.key}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
